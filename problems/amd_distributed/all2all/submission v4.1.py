...
class PyTorchAllToAll:
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        ...
        # Avoid building t_ids/k_ids: derive via divmod on original flat indices
        sorted_tokens = torch.div(order, k, rounding_mode='floor')
        sorted_ks = (order - sorted_tokens * k).to(torch.int32)
        sorted_tokens = sorted_tokens.to(torch.long)
        sorted_experts = flat_experts.gather(0, order).to(torch.int32)

        nsend = order.numel()
        # Use direct gather to avoid extra copy; it allocates once and is faster in practice
    ...
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor, recv_meta: torch.Tensor, expert_y_flat: torch.Tensor):
        ...
        # Recompute counts of messages to return to each src_rank
        ...
        order = torch.argsort(dst_ranks)
        nsend = order.numel()
        # If within local capacity, reuse prealloc; otherwise allocate ephemeral
        if nsend <= self.max_local_send:
            self._send_buf_full[:nsend].copy_(expert_y_flat.index_select(0, order))
            send_buf = self._send_buf_full[:nsend]
            send_meta = self._send_meta_full[:nsend]
            send_meta.copy_(recv_meta.index_select(0, order))
        else:
