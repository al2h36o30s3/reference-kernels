...
class PyTorchAllToAll:
    ...
    def __init__(self, cfg, rank: int, world_size: int):
        ...
        self._order_buf = torch.empty(max_total_tokens, dtype=torch.long, device=device)
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        ...
        # token ids and expert-k ids
        t_ids = torch.arange(num_tokens, device=device, dtype=torch.int32).repeat_interleave(k)
        k_ids = torch.arange(k, device=device, dtype=torch.int32).repeat(num_tokens)
        ...
        # stable ordering by rank for coalesced gather
    ...
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor, recv_meta: torch.Tensor, expert_y_flat: torch.Tensor):
        ...
        order = torch.argsort(dst_ranks, stable=True)
        send_buf = expert_y_flat.gather(0, order.unsqueeze(1).expand(-1, expert_y_flat.shape[1]))
        send_meta = recv_meta.gather(0, order.unsqueeze(1).expand(-1, self.META_DIM))
